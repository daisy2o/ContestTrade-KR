...
- **전체 KR 백테스트 흐름**: 구동 검증됨 (2026-09-24) — 데이터 소스 3종(DART·Factiva·텔레그램)·재생 하네스(backtest_runner)·채점(evaluation/score_signals)까지 실 API 5거래일 완주. 콘테스트(C3) 배선은 원본의 죽은 코드 상태 그대로 — 다음 개발 대물
- **파서의 실LLM 출력 견고성**: 합성 샘플만 통과 — 첫 실행 출력을 golden 픽스처로 승격 예정
- **FDR 시세의 수정주가 여부**: 미확인 (기업행위 채점 정책 D8과 함께)
- **콘테스트 모듈(원본)**: 배선 미완 + 자체 결함 확인됨 — `research_contest.py:311`이 존재하지 않는 `predict_signal_scores` 호출, predictor 초기화가 학습 진입 차단, reward 채점이 달력일+CN-Stock 하드코딩. "복구" 수준 공수 필요
