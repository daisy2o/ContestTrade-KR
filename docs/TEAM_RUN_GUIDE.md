...
>
> **이 가이드는 새 환경에서 처음부터 설치·실행해 검증한 것이 아니다.**
> 문서를 고치고 새로 클론해 파일 상태만 확인했다.
> **막히는 곳이 나오면 알려 달라** — 그게 이번 실행 확인의 목적이다.
...
### ⚠️ Factiva는 유료 구독 데이터입니다 — 팀 밖으로 전달하지 마세요

받는 데이터는 **두 개**입니다.

| 파일 | 내용 | 두는 곳 |
|---|---|---|
| `telegram_research.sqlite` | 증권사 리서치 텔레그램 4채널 | `data_collection/data/telegram/` |
| `factiva_news.sqlite` | Factiva 뉴스 | `data_collection/data/factiva/` |

> 🔒 **Factiva는 유료 구독에서 추출한 자료입니다.**
> **이 팀 세 명 밖으로 전달하지 마세요** — 다른 조, 외부, 공개 저장소, 클라우드
> 공유 링크 모두 포함입니다. 저장소에도 올리지 않습니다(`.gitignore`에 있음).
> 본인 KAIST 계정으로 직접 받을 수 있으면 그게 더 깔끔합니다
> (`data_collection/factiva_ingest.py`).

---

`파일럿_실행데이터.zip`을 받아 저장소 루트에서 푼다.
...
설정 파일은 예시를 복사한다(1절에서 이미 했으면 넘어간다).

```bash
cp config_kr.example.yaml config_kr.yaml
```

> **Factiva 없이 돌려야 할 때**(본인이 뉴스 파일을 못 쓰는 상황)는
> `cp config_kr.no-factiva.example.yaml config_kr.yaml`. 끝까지 돌아가지만
> 배경 정보가 절반으로 줄어 **출력이 공통 결과와 달라집니다** — 환경 확인용으로만
> 쓰세요.
...
# telegram  fc2ffc802c91692b… 로 시작하면 같은 파일
# factiva    1ca06ab5214b587c…
```
...
- 소스가 **3개**(`kr_dart_disclosure`, `kr_factiva_news`, `kr_telegram_research`) 나오는가
- 배경 정보가 **수천 자**인가 (수백 자면 팩터가 비었다는 뜻)
...
| `Factiva DB가 없습니다` | `factiva_news.sqlite`가 위 경로에 없음 |
