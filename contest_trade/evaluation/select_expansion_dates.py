...
풍부도 분류 (입력만 사용) — **'관련 메시지 수' 지표이지 밀도가 아니다**:
  계산식: as-of 창(판단일 직전 2일)의 텔레그램 메시지 중
          `tag_stock_codes(text) != [] AND 리서치 신호어 정규식 일치`
          인 **메시지 개수**. 전체 메시지 수로 나누지 않는다.
  상·하 구분: 선정된 10일의 그 개수 중앙값 이상이면 '상'.
  실험 중에는 이 분류를 바꾸지 않는다.
